...
export const pixKeyProvider = [
  {
    ...
    useFactory: (dataSource: DataSource) =>
      dataSource.getRepository(PixKeyModel),
    inject: ['DATA_SOURCE'],
  },
  {
    provide: 'PIX_KEY_REPOSITORY',
    useFactory: (repository: Repository<PixKeyModel>) =>
      new PixKeyRepository(repository),
    inject: ['PIX_KEY_MODEL'],
  },
  {
    provide: 'PIX_KEY_USE_CASE_CREATE',
    useFactory: (repository: IPixKeyRepository) =>
      new UseCaseCreate.CreateUseCase(repository),
